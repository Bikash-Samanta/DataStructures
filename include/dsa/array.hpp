...
namespace dsa {
	...
	public:
		...
		constexpr void push_back(ConstReference _Arg)noexcept {
			if (_Siz >= _Cap)
				_Reallocate(_Siz);
			if constexpr (std::is_fundamental_v<_Ty> || std::is_pointer_v<_Ty>)
				_Ptr[_Siz] = _Arg;
			else construct(_Ptr + _Siz, _Arg);
			++_Siz;
		}

		template<typename U = _Ty, typename = std::enable_if_t<!std::is_fundamental_v<U> && !std::is_pointer_v<U>>>
		constexpr void push_back(_Ty&& _Arg)noexcept {
			if (_Siz >= _Cap)
				_Reallocate(_Siz);
			if constexpr (std::is_fundamental_v<_Ty> || std::is_pointer_v<_Ty>)
				_Ptr[_Siz] = _Arg;
			else construct(_Ptr + _Siz, static_cast<std::remove_reference_t<_Ty>&&>(_Arg));
			++_Siz;
		}
		...
		virtual constexpr array& operator*=(const SizeType _Int)noexcept {
			...
			if (_NewSiz > _Cap)
				_Reallocate(_NewSiz);
			if constexpr (!std::is_fundamental_v<_Ty>)
	...
	protected:
		...
		inline auto _Reallocate(const SizeType _Size)noexcept {
			...
			if (_Ptr != nullptr) {
				...
				std::memcpy(_NewPtr, _Ptr, _NewSiz * sizeof(_Ty));
		...
		inline auto _Memset(Pointer _Start, Pointer _End, ConstReference _Val)noexcept {
			if constexpr (std::is_fundamental_v<_Ty>) {
				if constexpr (std::is_same_v<_Ty, char> || std::is_same_v<_Ty, unsigned char> || std::is_same_v<_Ty, signed char>) {
					std::memset(_Start, _Val, _End - _Start);
				} else {
					if (_Val != 0) {
						while (_Start != _End)
							*(_Start++) = _Val;
					} else { std::memset(_Start, 0, (_End  - _Start) * sizeof(_Ty));  }
				}
			} else {
				while(_Start != _End)
		...
		static inline auto _ConstructCopy(Pointer _Dst, ConstPointer _Src, SizeType _Size)noexcept{
			if constexpr (std::is_fundamental_v<_Ty> || std::is_pointer_v<_Ty>)
				std::memcpy(_Dst, _Src, _Size * sizeof(_Ty));
			else
		...
		inline constexpr auto _RightShift(const SizeType _Pos, const SizeType _Count)noexcept {
			...
			if (_NewSiz > _Cap) {
				...
				auto _NewPtr = _Allocator.allocate(_NewCap);
				std::memcpy(_NewPtr, _Ptr, _Pos * sizeof(_Ty));
				std::memcpy(_NewPtr + _Pos + _Count, _Ptr + _Pos, (_Siz - _Pos) * sizeof(_Ty));
				_Allocator.deallocate(_Ptr, _Cap);
				_Ptr = _NewPtr; _Cap = _NewCap;
			} else {
				std::memmove(_Ptr + _Pos + _Count, _Ptr + _Pos, (_Siz - _Pos) * sizeof(_Ty));
			}
			_Siz = _NewSiz;
		}
